...
void _confirmDeleteTransaction(BuildContext context, WidgetRef ref, TransactionModel transaction) {
  showDialog(
    context: context,
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                ...
                action: SnackBarAction(
                  label: 'BEKOR QILISH',
                  onPressed: () => ref.read(transactionProvider.notifier).addTransaction(transaction),
                ),
...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final monthTransactions = transactions.where((t) => t.date.year == _selectedMonth.year && t.date.month == _selectedMonth.month).toList();
    final monthIncome = monthTransactions.where((t) => t.type == 'income').fold(0.0, (s, t) => s + t.amount);
    final monthExpense = monthTransactions.where((t) => t.type == 'expense').fold(0.0, (s, t) => s + t.amount);
    final carryover = transactions.where((t) => !t.date.isAfter(monthEnd)).fold(0.0, (s, t) => s + (t.type == 'income' ? t.amount : -t.amount));

    final priorTransactions = transactions.where((t) => t.date.isBefore(startOfMonth)).toList();
    final hasPriorData = priorTransactions.isNotEmpty;
    final previousBalance = priorTransactions.fold(0.0, (s, t) => s + (t.type == 'income' ? t.amount : -t.amount));
          ...
          child: ListView(
            ...
            children: [
              ...
              SectionTitle(
                ...
                trailing: monthTransactions.isNotEmpty ? Text('${monthTransactions.length} ta', style: Theme.of(context).textTheme.labelSmall) : null,
              ),
              const SizedBox(height: 4),
              if (hasPriorData) FadeInItem(index: 0, child: _PreviousBalanceTile(amount: previousBalance)),
              if (monthTransactions.isEmpty && !hasPriorData)
                ...
                const Padding(padding: EdgeInsets.only(top: 8), child: EmptyState(icon: Icons.event_busy_rounded, title: "Bu oyda tranzaksiya yo'q"))
              else
                ...List.generate(monthTransactions.length, (index) {
                  ...
                  return FadeInItem(
                    ...
                    child: TransactionTile(
                      ...
                      onEdit: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => AddTransactionScreen(existing: t))),
                      onDelete: () => _confirmDeleteTransaction(context, ref, t),
                    ),
                  );
                }),
            ],
        ...
        Flexible(
          child: Column(
            ...
            children: [
              ...
              Text('PulNazorat', style: Theme.of(context).textTheme.titleLarge, overflow: TextOverflow.ellipsis),
            ],
          ),
        ),
        Row(
...
class _PreviousBalanceTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final color = isPositive ? AppTheme.brandIncome(context) : AppTheme.brandExpense(context);
    ...
    return AppCard(
      ...
      child: Row(
        children: [
          CircleAvatar(radius: 22, backgroundColor: color, child: const Icon(Icons.account_balance_wallet_outlined, color: Colors.white, size: 20)),
          const SizedBox(width: 12),
          Expanded(
            child: Text("O'tgan oydan qolgan balans", style: TextStyle(fontWeight: FontWeight.w700, fontSize: 14, color: Theme.of(context).colorScheme.onSurface)),
          ),
          Text('$sign${NumberFormat("#,##0").format(amount.abs())}', style: TextStyle(color: color, fontWeight: FontWeight.w700, fontSize: 14)),
        ],
      ),
    );
  }
...
class _BalanceCard extends StatelessWidget {
  ...
  const _BalanceCard({
    required this.selectedMonth, required this.isCurrentMonth, required this.carryover, required this.monthIncome, required this.monthExpense,
    required this.canGoNext, required this.slideDir, required this.onPrev, required this.onNext,
  });

  @override
  Widget build(BuildContext context) {
    // Joriy oyda "kelasi oyga o'tadigan mablag'", o'tgan oylarda esa doimiy "mavjud mablag'" yozuvi
    final label = isCurrentMonth ? "Oy oxiriga\nqolgan mablag'" : "Shu oy oxiridagi\nmavjud mablag'";
    ...
    return Container(
      ...
      child: Stack(
        children: [
          ...
          Positioned(right: -10, top: -10, child: _CoinShape(size: 90, label: '\$', opacity: 0.10)),
          Positioned(left: -6, bottom: -6, child: _CoinShape(size: 70, label: "so'm", opacity: 0.12)),
        ...
        children: [
          Row(
            ...
            children: [
                ...
                child: ClipRect(
                  child: AnimatedSwitcher(
                    ...
                    transitionBuilder: (child, animation) {
                      final offset = Tween<Offset>(begin: Offset(slideDir * 0.4, 0), end: Offset.zero).animate(animation);
              ...
              const SizedBox(width: 8),
              _ArrowButton(icon: Icons.chevron_right_rounded, onTap: canGoNext ? onNext : null),
            ],
          ),
          ...
          Row(
            children: [
              Expanded(child: ContrastStatChip(label: 'Bu oy kirim', value: monthIncome, color: AppTheme.emerald, icon: Icons.arrow_downward_rounded)),
              const SizedBox(width: 12),
              Container(width: 1, height: 34, color: Colors.white.withValues(alpha: 0.12)),
              const SizedBox(width: 12),
              Expanded(child: ContrastStatChip(label: 'Bu oy chiqim', value: monthExpense, color: AppTheme.coral, icon: Icons.arrow_upward_rounded)),
            ],
          ),
            ],
            ),
...
class _WavePatternPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final yOffset in [0.25, 0.42, 0.6]) {
      ...
      path.quadraticBezierTo(size.width * 0.25, baseY - 24, size.width * 0.5, baseY);
      path.quadraticBezierTo(size.width * 0.75, baseY + 24, size.width + 20, baseY);
...
class _CoinShape extends StatelessWidget {
  ...
  const _CoinShape({required this.size, required this.label, required this.opacity});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(label, style: TextStyle(color: Colors.white.withValues(alpha: opacity * 4), fontSize: size * 0.26, fontWeight: FontWeight.w700)),
    );
  }
}
...
class _Dot extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Container(width: size, height: size, decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.08), shape: BoxShape.circle));
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: Colors.white.withValues(alpha: disabled ? 0.06 : 0.18), shape: BoxShape.circle),
        child: Icon(icon, color: Colors.white.withValues(alpha: disabled ? 0.3 : 1), size: 26),
      ),
    );
  }
}