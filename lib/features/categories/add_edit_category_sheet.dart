...
class _AddEditCategorySheetState extends ConsumerState<AddEditCategorySheet> {
  ...
  void initState() {
    ...
    _selectedColor = widget.existing != null ? Color(widget.existing!.colorValue) : IconHelper.colorPalette.first;
  }
            ...
            children: [
              Center(
                child: Container(
                  ...
                  decoration: BoxDecoration(color: onSurface.withValues(alpha: 0.2), borderRadius: BorderRadius.circular(10)),
                ),
              ),
              Text(
                isEditing ? "Bo'limni tahrirlash" : "Yangi bo'lim qo'shish",
                style: TextStyle(fontSize: 19, fontWeight: FontWeight.w800, color: onSurface),
              ),
              ...
              Text('Rang', style: TextStyle(fontWeight: FontWeight.w700, color: onSurface, fontSize: 15)),
              ...
              Text('Ikonka', style: TextStyle(fontWeight: FontWeight.w700, color: onSurface, fontSize: 15)),
              const SizedBox(height: 10),
              SizedBox(
                height: 140,
                child: GridView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return GestureDetector(
                      ...
                      child: Container(
                        decoration: BoxDecoration(
                          color: selected ? _selectedColor.withValues(alpha: 0.2) : onSurface.withValues(alpha: 0.06),
                          borderRadius: BorderRadius.circular(12),
                          border: selected ? Border.all(color: _selectedColor, width: 1.5) : null,
                        ),
                        child: Icon(icon, color: selected ? _selectedColor : onSurface.withValues(alpha: 0.6), size: 20),
                      ),
                    );
                  },
                ),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  style: ElevatedButton.styleFrom(backgroundColor: AppTheme.brandPrimary(context)),
                  onPressed: _save,
  ...
  void _save() {
    ...
    if (name.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Bo'lim nomini kiriting")));
      return;
    }

    if (widget.existing != null) {
      final updated = CategoryModel(
        id: widget.existing!.id, name: name, iconCode: _selectedIcon,
        colorValue: _selectedColor.toARGB32(), type: widget.type, isDefault: widget.existing!.isDefault,
      );
      ref.read(categoryProvider.notifier).updateCategory(updated);
    } else {
      final newCategory = CategoryModel(
        id: const Uuid().v4(), name: name, iconCode: _selectedIcon,
        colorValue: _selectedColor.toARGB32(), type: widget.type, isDefault: false,
      );
